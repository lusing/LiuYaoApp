...
class LiuYao {
    ...
    fun checkYongShen() {
        ...
        if (this.isShi) {
            ...
            for (i in 0..5) {
                if (this.benGua.yaos[i].lq == this.yongShen) {
                    println2("用神在本卦第${i + 1}爻")
                    val yao = this.benGua.yaos[i]
                    println2(this.getYaoName(yao))
        ...
        if (yongshen != null) {
            var yao1 = yongshen!!
            if (isFuShen) {
                println2("用神为伏神${this.getYaoName(yao1.fuShen!!)}")
            } else {
                println2("用神为${this.getYaoName(yao1)}")
            }
    ...
    fun checkDeShi(whatShen: YongShen, yongShenWang: Int, shiYaoWang: Int): JiXiong {
        if (whatShen.yongShen == YongShen.YONG_SHEN) {
            //用神持世：按世爻旺衰来直接断吉凶
            println2("用神持世：按世爻旺衰来直接断吉凶")
            println2("为事体临身之象，说明所测的事情与求测人比较贴近，比较熟练，求测人有近水楼台的好处")
            println2("得：世爻在月日旺相有气，且又无动爻变爻来伤克。如果再遇到原神动而生世，则为源远流长之象；如遇到原神休囚受伤者，则为难以长久之象")
            println2("失：世或用爻有一个在月日休囚无气，或旬空月破，并受变动爻的克伤")
            if (shiYaoWang > 0) {
                println2("世爻旺，吉")
            ...
            println2("原神持世：自己追求目标，世旺用旺则以得断，有一方衰则断某方有问题，双方都衰则以失断")
            println2("说明求测人会一直费心着做事，而且自己在得到这件事前要先投入本钱与精力的。")
            println2("得：世爻在月日旺相有气，且又无动爻变爻来伤克。如果再遇到世爻动而生用，说明经过求测者的努力，也能成就其事")
            println2("失：世或用爻有一个在月日休囚无气，或旬空月破，并受变动爻的克伤")
            ...
            println2("仇神持世：世爻用神均中平以上为得，世强用衰为意愿强而目标差，世衰用强为能力不足而先补能力，世用双衰为失")
            println2("为事体克身之象，除了求财求物与出门游玩享乐求子者为吉象外，其它都是不利之象。它代表因为求测者的能力与环境不适应，而使所测之事对求测者造成压力，所测之事阻力重重，求测者有独木难支之感")
            println2("占治病去忧游玩求子时，用神旺相克世者，为得象")
            println2("占求财索物时，用爻世爻均旺相有气者")
            println2("世爻用爻间有动爻通关生世者，此为化煞生身，为成象")
            println2("仇神持世者，都为用神克世，所以除了以上三种类型外，仇神持世者都为事体不成，甚至是因事引祸之象")
            if (shiYaoWang < 0 && yongShenWang < 0) {
                println2("世爻衰，用神衰，凶")
                JiXiong(JiXiong.XIONG)
            } else {
            ...
            println2("忌神持世：世爻克用神，这是世爻追逐用神之意，除用神过衰外，一般为得。防用力过猛，世爻和用神均中平以上为得，世强用衰为意愿强而目标差，世衰用强为能力不足而先补能力，世用双衰为失")
            println2("为自身见阻之象，说明求测者根本没有素质与环境去适合自己想做之事，凡遇到忌神持世者，多数都是事件难成之象的")
            println2("占求财索物时")
            println2(
            ...
            println2("失：忌临身而多谋少成，所以除了以上四种类型外，忌神持世者一般都是多谋少成之象")
            if (shiYaoWang < 0 && yongShenWang < 0) {
                println2("世爻衰，用神衰，凶")
                JiXiong(JiXiong.XIONG)
            } else {
            ...
            println2("闲神持世：（用神生世爻）只要用神不为衰，则为得")
            println2("为事体生身之象，说明所测的事情比较能够帮助求测人，求测人容易在其中获得好处，求测人有先天有利的环境与条件，可以称得上是唾手可得")
            println2("得：世爻在月日旺相有气，且又无动爻变爻来伤克。如果再遇到用爻动而生世，则更为大吉之象")
            println2("失：世或用爻有一个在月日休囚无气，或旬空月破，并受变动爻的克伤")
            if (yongShenWang < 0) {
                println2("用神衰，凶")
                JiXiong(JiXiong.XIONG)
    ...
    fun checkAll() {
        ...
        for (i in 0..5) {
            ...
            if (yao.isChange) {
                ...
                println2(this.getYaoName(bianYao))
                if (yao.bianYao == null) {
                    yao.bianYao = bianYao
                }
                this.checkBian(yao)
    ...
    fun checkShengKe(yao: Yao, wang: Int, fuYao: Int = 0): Pair<Int, Int> {
        ...
        for (i in 0..5) {
            if (this.benGua.yaos[i].isChange) {
                if (this.benGua.yaos[i].naZhi.isHe(yao.naZhi)) {
                    println2("动爻${this.getYaoName(this.benGua.yaos[i])}合爻${this.getYaoName(yao)}")
                } else if (this.benGua.yaos[i].naZhi.isHe(yao.naZhi)) {
                    println2("动爻${this.getYaoName(this.benGua.yaos[i])}冲爻${this.getYaoName(yao)}")
                } else if (this.benGua.yaos[i].naZhi.isSheng(yao.naZhi)) {
                    shengs++;
                    println2("动爻${this.getYaoName(this.benGua.yaos[i])}生爻${this.getYaoName(yao)}")
                } else if (this.benGua.yaos[i].naZhi.isKe(yao.naZhi)) {
                    println2("动爻${this.getYaoName(this.benGua.yaos[i])}克爻${this.getYaoName(yao)}")
                    kes++;
                }
            }
        }
        ...
        if (wang >= 0) {
            if (shengs > 0) {
                if (wang == 100) {
                    println2("用神旺极：刚脆则折，凶。")
                } else {
                    println2("用旺逢生：锦上添花，吉")
                }
            }
            if (kes > 0) {
                println2("用旺受克：易有不利，凶")
            } else if (shengs == 0) {
                println2("用旺无克：平平无奇，平")
            }
        } else {
            if (shengs > 0) {
                println2("用衰逢生：危中有救，吉")
            } else if (kes > 0) {
                println2("用衰受克：屋漏逢雨，凶")
            } else if (shengs == 0) {
                if (wang <= -70) {
                    println2("用神弱极，衰极无救,凶")
                } else {
                    println2("用衰无克：勉力支撑，吉")
                }
            }
        }
